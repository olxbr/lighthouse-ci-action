...
if [[ "${JSON_COMPARE_RESULTS}" != false ]]; then
    ...
    for previous_url in $previous_urls; do
        ...
        _log "${space_hex}${C_WHT_NO_BOLD}🆄🆁🅻${C_END} $(jq -r ".[$idx].url" <<< ${recent_results})"
        _log "┌$(eval printf '─%.0s' {3..$coll_length})┐"
        
        ## for each summary compare to the new version
        _log "| \x20\x20🅢 ${C_WHT}Summary (Difference)${C_END}\x09" $(($coll_length+7)) │
        ...
        _log "| \x20\x20🅜 ${C_WHT}Metrics (Difference)${C_END}\x09" $(($coll_length+7)) │
