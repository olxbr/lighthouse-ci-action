...
if [[ "${JSON_COMPARE_RESULTS}" != false ]]; then
    ...
    for previous_url in $previous_urls; do
        ...
        for s_key in $previous_summary_keys; do
            ...
            [[ $res_value -gt 0 ]] && _log "|${space_hex}${gre_inc_arrow} Increase in ${bold_key} (${res_value}%)" $(($coll_length+22)) │
            [[ $res_value -lt 0 ]] && _log "|${space_hex}${red_dec_arrow} Decrease in ${bold_key} (${res_value}%)" $(($coll_length+22)) │
