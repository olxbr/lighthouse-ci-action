...
for previous_url in $previous_urls; do
    ...
    for s_key in $previous_summary_keys; do
        ...
        [[ $res_value -lt 0 ]] &&
            aggregate_reports=$(jq ".[$idx].summary.$s_key=\"$report_metric(${red_mark}${res_value}%)\"" <<< $aggregate_reports) &&
            log_line="|      ${red_mark}\x09Decrease in ${bold_key} (${res_value}%)"

        [[ $res_value -eq 0 ]] &&
            aggregate_reports=$(jq ".[$idx].summary.$s_key=\"$report_metric(${eql_mark}${res_value}%)\"" <<< $aggregate_reports) &&
    ...
    for m_key in $previous_metrics_keys; do
        ...
        [[ $res_value -gt 0 ]] &&
            aggregate_reports=$(jq ".[$idx].summary.$m_key=\"$report_metric(${red_mark}${res_value}${metric_unit})\"" <<< $aggregate_reports) &&
            log_line="|      ${red_mark}\x09Increase time in ${bold_key} (${res_value} ${metric_unit})"

        [[ $res_value -lt 0 ]] &&
            aggregate_reports=$(jq ".[$idx].summary.$m_key=\"$report_metric(${gre_mark}${res_value}${metric_unit})\"" <<< $aggregate_reports) &&
            log_line="|      ${gre_mark}\x09Decrease time in ${bold_key} (${res_value} ${metric_unit})"

        [[ $res_value -eq 0 ]] &&
            aggregate_reports=$(jq ".[$idx].summary.$m_key=\"$report_metric(${eql_mark}${res_value}${metric_unit})\"" <<< $aggregate_reports) &&
