...
for previous_url in $previous_urls; do
    ...
    for s_key in $previous_summary_keys; do
        ...
        [[ $res_value -gt 0 ]] &&
            aggregate_reports=$(jq ".[$idx].summary.$s_key=\"$report_metric(${res_value}%)\"" <<< $aggregate_reports) &&
    ...
    for m_key in $previous_metrics_keys; do
        ...
        [[ $res_value -gt 0 ]] &&
            aggregate_reports=$(jq ".[$idx].summary.$m_key=\"$report_metric(${res_value}${metric_unit})\"" <<< $aggregate_reports) &&
