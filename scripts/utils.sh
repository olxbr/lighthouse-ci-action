...
function _check_url_availability() {
    ...
    [[ ${#urls[@]} -eq 0 ]] && _log warn "No urls to check availability" && return

    _log debug "Checking availability of ${#urls[@]} urls [${urls[@]}] with timeout: ${timeout}s - retries: ${retries} - sleep: ${sleep}s"
